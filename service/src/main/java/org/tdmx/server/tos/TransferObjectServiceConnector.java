...
/**
 * Handles inbound RPC calls from WS clients to transfer inbound data.
 * 
 * @author Peter
 *
 */
public class TransferObjectServiceConnector implements Manageable, TransferObjectServiceProxy.BlockingInterface {
	...
	// -------------------------------------------------------------------------
	private static final Logger log = LoggerFactory.getLogger(TransferObjectServiceConnector.class);
	...
	public void start(Segment segment, List<WebServiceApiName> apis) {
		...
		BlockingService controlServiceProxy = TransferObjectServiceProxy.newReflectiveBlockingService(this);
		serverFactory.getRpcServiceRegistry().registerService(controlServiceProxy);

		// Configure the server.
		ServerBootstrap bootstrap = new ServerBootstrap();
		NioEventLoopGroup boss = new NioEventLoopGroup(acceptorThreads, new NamedThreadFactory("acceptor"));
		NioEventLoopGroup workers = new NioEventLoopGroup(ioThreads, new NamedThreadFactory("worker"));
	...
	@Override
	public TransferResponse transfer(RpcController controller, TransferRequest request) throws ServiceException {
		// some other WS has transferred us an object
		boolean result = false;
		if (objectReceiver != null) {
			result = objectReceiver.transferObject(request.getSessionId(), mapApi(request.getApiName()),
					request.getTransferType(), mapAttributes(request.getAttributeList()));
		}
		TransferResponse.Builder response = TransferResponse.newBuilder();
	...
	public TransferObjectReceiver getObjectReceiver() {
		return objectReceiver;
	}

	public void setObjectReceiver(TransferObjectReceiver objectReceiver) {
