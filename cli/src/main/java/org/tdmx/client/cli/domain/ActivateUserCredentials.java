...
@Cli(name = "user:activate", description = "activates a user at the service provider.")
public class ActivateUserCredentials implements CommandExecutable {
	...
	public void run(CliPrinter out) {
		...
		if (searchUserResponse.isSuccess() && !searchUserResponse.getUsers().isEmpty()) {
			...
			org.tdmx.core.api.v01.zas.ModifyUser modifyUserRequest = new org.tdmx.core.api.v01.zas.ModifyUser();
			modifyUserRequest.setUserIdentity(id);
			modifyUserRequest.setStatus(CredentialStatus.ACTIVE);
			modifyUserRequest.setSessionId(sessionResponse.getSession().getSessionId());

			org.tdmx.core.api.v01.zas.ModifyUserResponse modifyUserResponse = zas.modifyUser(modifyUserRequest);
			if (modifyUserResponse.isSuccess()) {
				out.println(
						"User " + username + " with fingerprint " + userCertificate.getFingerprint() + " reactivated.");
			} else {
				out.println("Unable to modify user ", modifyUserResponse.getError());
			}

		} else {
			org.tdmx.core.api.v01.zas.CreateUser activateUserRequest = new org.tdmx.core.api.v01.zas.CreateUser();
			activateUserRequest.setUserIdentity(id);
			activateUserRequest.setStatus(CredentialStatus.ACTIVE);
			activateUserRequest.setSessionId(sessionResponse.getSession().getSessionId());

			org.tdmx.core.api.v01.zas.CreateUserResponse activateUserResponse = zas.createUser(activateUserRequest);
			if (activateUserResponse.isSuccess()) {
				out.println(
						"User " + username + " with fingerprint " + userCertificate.getFingerprint() + " activated.");
			} else {
				out.println("Unable to create user ", activateUserResponse.getError());
			}
		}
	}
