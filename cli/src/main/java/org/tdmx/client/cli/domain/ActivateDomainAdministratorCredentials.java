...
public class ActivateDomainAdministratorCredentials implements CommandExecutable {
	...
	public void run(PrintStream out) {
		...
		org.tdmx.core.api.v01.zas.CreateAdministrator createAdminRequest = new org.tdmx.core.api.v01.zas.CreateAdministrator();
		...
		createAdminRequest.setAdministratorIdentity(id);
		createAdminRequest.setStatus(CredentialStatus.ACTIVE);
		createAdminRequest.setSessionId(sessionResponse.getSession().getSessionId());

		org.tdmx.core.api.v01.zas.CreateAdministratorResponse deleteAdminResponse = zas
				.createAdministrator(createAdminRequest);
		if (deleteAdminResponse.isSuccess()) {
			out.println(
					"Administrator for domain " + domain + " with fingerprint " + dac.getFingerprint() + " activated.");
		} else {
