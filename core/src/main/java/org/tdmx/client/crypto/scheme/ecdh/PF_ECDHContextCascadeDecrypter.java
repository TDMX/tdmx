...
public class PF_ECDHContextCascadeDecrypter implements Decrypter {
	...
	public PF_ECDHContextCascadeDecrypter(KeyPair ownSigningKey, PublicKey otherSigningKey, byte[] passphrase,
			KeyPair sessionKey, boolean rsaEnabled, StreamCipherAlgorithm keyEncryptionCipher,
			StreamCipherAlgorithm innerPayloadCipher, StreamCipherAlgorithm outerPayloadCipher) throws CryptoException {
	...
	public InputStream getInputStream(InputStream encryptedData, byte[] encryptionContext) throws CryptoException {
		...
		byte[] innerPayloadIv = ByteArray.subArray(payloadKeyBytes, innerPayloadCipher.getKeyLength(),
		...
		byte[] outerPayloadIv = ByteArray.subArray(payloadKeyBytes, innerPayloadCipher.getKeyLength()
				+ innerPayloadCipher.getIvLength() + outerPayloadCipher.getKeyLength(),
				outerPayloadCipher.getIvLength());
		...
		// TODO System.out.println("Outer Payload KEY: " + ByteArray.asHex(outerPayloadKey));
		// TODO System.out.println("Outer Payload IV: " + ByteArray.asHex(outerPayloadIv));
