...
public final class SearchExpressionParser {
	...
	/**
	 * ValueTypeFormatter uses the system's Locale to format Time,DateTime,Date and Number values.
	 */
	 ...
	 * 
	 * RangedText ( contains .. ) TimeRange ( Time..Time, ..Time, Time.. ) DateTimeRange ( DateTime..DateTime,
	 * ..DateTime, DateTime.. ) DateRange ( Date..Date, ..Date, Date.. ) NumberRange ( Number..Number, ..Number,
	 * Number.. )
	 * 
	 */
	...
	private boolean parseNumberText(String text, SearchExpression exp) {
		...
		if (numberRange != null) {
			...
			exp.add(FieldType.Number, new NumberRangeNumberMatch(MatchValueNormalizer.getNumber(numberRange.from),
					MatchValueNormalizer.getNumber(numberRange.to)));
			exp.add(FieldType.Text,
					new TextLikeOrMatch(MatchValueNormalizer.getStringNumberList(numberRange.from, numberRange.to)));
			return true;
		}
	...
	private boolean parseCalendarText(String text, SearchExpression exp) {
		...
		if (timeRange != null) {
			...
			exp.add(FieldType.Time, new TimeRangeTimeMatch(MatchValueNormalizer.getTime(timeRange.from),
					MatchValueNormalizer.getTime(timeRange.to)));
			exp.add(FieldType.DateTime, new TimeRangeDateTimeMatch(MatchValueNormalizer.getTime(timeRange.from),
					MatchValueNormalizer.getTime(timeRange.to)));
			exp.add(FieldType.Text,
					new QuotedTextOrMatch(MatchValueNormalizer.getStringTimeList(timeRange.from, timeRange.to)));
			return true;
		}
		CalendarRangeHolder dateTimeRange = ValueTypeParser.parseDateTimeRange(text);
		if (dateTimeRange != null) {
			exp.valueType = ValueType.DateTimeRange;
			exp.add(FieldType.DateTime,
					new DateTimeRangeDateTimeMatch(MatchValueNormalizer.getDateTimeTS(dateTimeRange.from),
							MatchValueNormalizer.getDateTimeTS(dateTimeRange.to)));
			exp.add(FieldType.Date, new DateTimeRangeDateMatch(MatchValueNormalizer.getDate(dateTimeRange.from),
					MatchValueNormalizer.getDate(dateTimeRange.to)));
			exp.add(FieldType.Text,
					new QuotedTextOrMatch(MatchValueNormalizer.getStringTimeList(dateTimeRange.from, dateTimeRange.to)));
			return true;
		}
		CalendarRangeHolder dateRange = ValueTypeParser.parseDateRange(text);
		if (dateRange != null) {
			exp.valueType = ValueType.DateRange;
			exp.add(FieldType.DateTime, new DateRangeDateTimeMatch(MatchValueNormalizer.getDate(dateRange.from),
					MatchValueNormalizer.getDate(dateRange.to)));
			exp.add(FieldType.Date, new DateRangeDateMatch(MatchValueNormalizer.getDate(dateRange.from),
					MatchValueNormalizer.getDate(dateRange.to)));
			exp.add(FieldType.Text,
					new QuotedTextOrMatch(MatchValueNormalizer.getStringDateList(dateRange.from, dateRange.to)));
			return true;
		}
